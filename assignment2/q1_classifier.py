...
class SoftmaxModel(Model):
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        W= tf.Variable(tf.constant(0.0,tf.float32,shape=[self.config.n_features,self.config.n_classes]))
        b= tf.Variable(tf.constant(0.0,tf.float32,shape=[self.config.n_classes]))
