...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        for i in xrange(len(sentence)):
            ...
            elif i-window_size<0 and i+window_size<len(sentence):
                words=[start]*(window_size-i)+sentence[:i+window_size+1]
            elif i-window_size>=0 and i+window_size>=len(sentence):
                words=sentence[i-window_size:]+[end]*(i+window_size-len(sentence)+1)
            else:
                words=[start]*(window_size-i)+sentence+[end]*(i+window_size-len(sentence)+1)
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable("U", shape=[self.config.hidden_size, self.config.n_classes],
                             initializer=tf.contrib.layers.xavier_initializer())
        b1=tf.Variable(tf.constant(0.0,tf.float32,shape=[self.config.hidden_size]))
        h=tf.nn.relu(tf.matmul(x,W)+b1)
        h_drop=tf.nn.dropout(h,keep_prob=1-self.config.dropout)
        b2=tf.Variable(tf.constant(0.0,tf.float32,shape=[self.config.n_classes]))
