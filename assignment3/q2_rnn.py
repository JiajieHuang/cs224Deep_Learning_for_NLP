...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence)<max_length:
            new_sentence=sentence+[zero_vector]*(max_length-N)
            new_labels=labels+[zero_label]*(max_length-N)
            masking=[True]*N+[False]*(max_length-N)
        else:
            new_sentence=sentence[:max_length]
            new_labels=labels[:max_length]
            masking=[True]*max_length
        #new_labels_flat=[val for sublist in new_labels for val in sublist]
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        b2=tf.Variable(tf.constant(0.0,tf.float32,shape=[Config.n_classes]))
    ...
    def add_loss_op(self, preds):
        ...
        """
        ### YOUR CODE HERE (~2-4 lines)
        loss_without_mask=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels_placeholder,logits=preds)
        loss=tf.reduce_mean(tf.boolean_mask(loss_without_mask,self.mask_placeholder))
        ### END YOUR CODE
        return loss
